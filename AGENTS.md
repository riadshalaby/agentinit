...
## PR Policy
- Feature PRs use `agentinit pr`.
- `agentinit pr` writes the Summary, Breaking Changes, Included Commits, and Test Plan sections for feature PRs.
...
- Mode is selected by the launcher prompt/context:
  - Cycle bootstrap:
    - `agentinit cycle start <branch-name>`
  - Convenience wrappers:
    - `agentinit plan [agent] [agent-options...]` (default agent from `.ai/config.json`, fallback: `claude`)
    - `agentinit implement [agent] [agent-options...]` (default agent from `.ai/config.json`, fallback: `codex`)
    - `agentinit review [agent] [agent-options...]` (default agent from `.ai/config.json`, fallback: `claude`)
    - `agentinit po [agent] [agent-options...]` (cross-platform launcher for the PO orchestration session)
...
- Auto mode:
  - you start the PO session with `agentinit po`
  - the PO session uses the `agentinit` MCP server to run the post-planning loop by coordinating implementer and reviewer sessions for the same task flow
...
- Start a new development cycle with `agentinit cycle start <branch-name>`.
...
- PO session:
  - launched with `agentinit po [agent]` (default agent: `claude`)
  - uses MCP tools internally (`session_start`, `session_run`, `session_get_output`, `session_status`, `session_list`, `session_stop`, `session_reset`, `session_delete`) to coordinate role sessions
...
- Implementer session:
  ...
  - `agentinit cycle end [VERSION]`
    ...
    - if a version argument is provided (for example `agentinit cycle end 0.7.0`), add `Release-As: x.y.z` to the commit body; if no version is supplied, ask the user for it before committing
    - then run `agentinit pr` to update the PR
...
- Commit behavior by role:
  ...
  - `agentinit cycle end` commits any remaining dirty `.ai/` artifacts as the cycle-close commit and always includes a `Release-As: x.y.z` footer.
