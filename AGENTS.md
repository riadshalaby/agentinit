...
- Commit behavior by role:
  ...
  - `implement` role must stage all changes and create a Conventional Commit after validations pass. The `implement` role also creates the cycle-close commit for `.ai/` artifacts via `finish_cycle`.
