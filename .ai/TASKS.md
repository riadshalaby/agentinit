...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| T-001 | Fix GoReleaser: chain in release-please.yml, remove release.yml | claude | codex | claude | ready_for_test | release-please.yml has goreleaser job conditioned on release_created; release.yml removed; valid YAML | go fmt ./...; go vet ./...; go test ./...; ruby YAML parse | test |
| T-002 | Claude settings templates: scaffold .claude/settings.json and settings.local.json | claude | codex | claude | ready_for_implement | agentinit init produces both files; settings.local.json has validation + git permissions; go test passes | n/a | implement |
| T-003 | Tool access parity: expand settings.local.json with full toolchain per overlay | claude | codex | claude | ready_for_implement | Go/Node/Java projects get correct tool permissions; base tools (gh, rg, fd, bat, jq, sg, fzf) present; go test passes | n/a | implement |
