...
| T-004 | Add `agentinit update` command with manifest-based and fallback file management | claude | codex | claude | ready_for_review | `agentinit update` refreshes managed files, preserves user content, supports `--dry-run`, works with and without manifest; tests pass | PASS | review |
| T-005 | Manual validation cycle with restructured files | claude | — | claude | ready_for_implement | Full cycle completes; conventional commits, no Co-Authored-By, validations run; no `.ai/AGENTS.md` references | n/a | implement |
