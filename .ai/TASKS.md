...
| T-003 | Search-strategy skill layer in .ai/prompts/ and templates | claude | codex | claude | ready_for_review | search-strategy.md exists in own project and as template; all agent prompts reference it; engine_test covers new file and references; go test passes | `go vet ./...`; `go test ./...` | review |
| T-004 | Add ast-grep, fzf, tree-sitter to prereq registry + CLAUDE.md rules | claude | codex | claude | todo | Registry returns 10 tools; new tools are Required:false; wizard shows them without blocking; CLAUDE.md has conditional rules for ast-grep and fzf; tests cover detection and install-plan; go test passes | n/a | implement |
