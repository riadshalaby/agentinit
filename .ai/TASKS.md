...
| T-010 | Rename binary from `agentinit` to `aide` (module path unchanged) | ready_for_implement | `go install github.com/riadshalaby/agentinit/aide@latest` produces `aide`; MCP server name is `"aide"`; generated settings use `mcp__aide__*`; module path and imports unchanged; `go test ./...` passes | n/a | implement |
