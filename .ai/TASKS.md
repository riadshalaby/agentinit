...
| T-003 | Add GoReleaser config and release workflow | claude | codex | claude | ready_for_review | `.goreleaser.yml` and `.github/workflows/release.yml` exist; workflow triggers on `v*` tags; builds linux/darwin amd64/arm64 | `.goreleaser.yml`; `.github/workflows/release.yml`; `go fmt ./...`; `go vet ./...`; `go test ./...` | review |
