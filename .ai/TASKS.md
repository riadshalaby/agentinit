...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| T-001 | Remove --workflow flag, Workflow field, constants, helpers, and all call-site plumbing | claude | codex | claude | ready_for_test | `--workflow` flag gone; `ProjectData.Workflow` removed; `scaffold.Run` and `wizard.Run` no longer accept workflow param; wizard UI has no workflow select; code compiles | `go fmt ./...`; `go test ./...`; `go vet ./...` | test |
