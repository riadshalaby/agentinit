...
| T-006 | Restructure AGENTS.md files with Hard Rules block at top | claude | codex | claude | done | Hard Rules section is the first `##` after `# AGENTS` in both the scaffold template (`AGENTS.md.tmpl`) and this project's own `.ai/AGENTS.md`; contains no-Co-Authored-By, prefer-rg, prefer-fd, prefer-bat rules; duplicates removed from original locations; `go test ./...` green | `go fmt ./...`; `go test ./...`; `go vet ./...` | none |
| T-007 | Sync project AGENTS files with updated templates | claude | codex | claude | ready_for_review | `AGENTS.md` references `po.md`; `.ai/AGENTS.md` has `ai-po.sh` in wrappers, `## Runtime Modes` section, auto mode in Persistent Session Workflow, and PO session commands; content matches template output | `go fmt ./...`; `go test ./...`; `go vet ./...`; `.ai/prompts/po.md` and `scripts/ai-po.sh` present locally | review |
