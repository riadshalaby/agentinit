...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| T-001 | Replace hardcoded version with `runtime/debug.ReadBuildInfo()` | claude | codex | claude | ready_for_test | `go run . --version` prints `(dev)`; `go install @tag` prints release version; `go vet` and `go test` pass | `go fmt ./...`; `go vet ./...`; `go test ./...`; `go run . --version` prints `agentinit version (dev)` | test |
| T-002 | Add release-please configuration | claude | codex | claude | ready_for_implement | `.release-please-manifest.json`, `release-please-config.json`, `.github/workflows/release-please.yml` exist; workflow triggers on push to main | n/a | implement |
