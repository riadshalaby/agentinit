...
| T-002 | Remove template conditionals so PO artifacts are always rendered | claude | codex | claude | changes_requested | `po.md.tmpl` and `ai-po.sh.tmpl` have no `{{if}}` guard; `AGENTS.md.tmpl` always references `po.md`; `README.md.tmpl` has no workflow conditional | `go fmt ./...`; `go test ./...`; `go vet ./...` | implement |
