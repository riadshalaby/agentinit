...
| T-002 | Inline 5 critical rules into each role prompt and matching templates | claude | codex | claude | ready_for_test | Each prompt has `## Critical Rules` section with 5 inlined rules; single `AGENTS.md` reference for full ruleset; `.tmpl` files match | PASS_WITH_NOTES | test |
