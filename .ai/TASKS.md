...
| T-004 | Add ast-grep, fzf, tree-sitter to prereq registry + CLAUDE.md rules | claude | codex | claude | ready_for_review | Registry returns 10 tools; new tools are Required:false; wizard shows them without blocking; CLAUDE.md has conditional rules for ast-grep and fzf; tests cover detection and install-plan; go test passes | `go vet ./...`; `go test ./...` | review |
| T-005 | Sync planner workflow rule in templates: all planned tasks → ready_for_implement | claude | codex | claude | ready_for_implement | CLAUDE.md.tmpl and planner.md.tmpl say "all newly planned tasks" instead of "selected first task"; engine_test verifies the wording; go test passes | n/a | implement |
