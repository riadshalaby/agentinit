...
| T-007 | Sync project AGENTS files with updated templates | claude | codex | claude | ready_for_test | `AGENTS.md` references `po.md`; `.ai/AGENTS.md` has `ai-po.sh` in wrappers, `## Runtime Modes` section, auto mode in Persistent Session Workflow, and PO session commands; content matches template output | `go fmt ./...`; `go test ./...`; `go vet ./...`; `.ai/prompts/po.md` and `scripts/ai-po.sh` present locally | test |
