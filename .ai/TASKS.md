...
| T-006 | Restructure AGENTS.md files with Hard Rules block at top | claude | codex | claude | ready_for_review | Hard Rules section is the first `##` after `# AGENTS` in both the scaffold template (`AGENTS.md.tmpl`) and this project's own `.ai/AGENTS.md`; contains no-Co-Authored-By, prefer-rg, prefer-fd, prefer-bat rules; duplicates removed from original locations; `go test ./...` green | `go fmt ./...`; `go test ./...`; `go vet ./...` | review |
