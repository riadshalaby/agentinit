...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| T-001 | Replace hardcoded version with `runtime/debug.ReadBuildInfo()` | claude | codex | claude | ready_for_review | `go run . --version` prints `(dev)`; `go install @tag` prints release version; `go vet` and `go test` pass | `go fmt ./...`; `go vet ./...`; `go test ./...`; `go run . --version` | review |
