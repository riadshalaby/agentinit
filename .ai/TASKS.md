...
| T-003 | Add GoReleaser config and release workflow | claude | codex | claude | ready_for_test | `.goreleaser.yml` and `.github/workflows/release.yml` exist; workflow triggers on `v*` tags; builds linux/darwin/windows amd64/arm64 | `.goreleaser.yml`; `.github/workflows/release.yml`; tester found `.goreleaser.yml` omitted Windows targets; rework added `windows` to `goos`; `go fmt ./...`; `go vet ./...`; `go test ./...` | test |
