...
func TestReviewCommandRefusesInLiteProfile(t *testing.T) {
	...
	for _, snippet := range []string{"Lite profile:", "`next_task`", "`aide profile full`"} {
...
func TestReviewCommandRunsInFullProfile(t *testing.T) {
	originalGetWorkingDir := getWorkingDir
	originalLoadLaunchConfig := loadLaunchConfig
	originalLaunchRole := launchRole
	t.Cleanup(func() {
